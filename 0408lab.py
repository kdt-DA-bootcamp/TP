...
# SIMILAR_GAMES 데이터베이스 연결 함수 (기존 유지)
@st.cache_data
def fetch_similar_games(game_app_id):
    ...
    try:
        ...
        if results:
            ...
            # recommended_app_id를 int64로 강제 변환
            df["recommended_app_id"] = df["recommended_app_id"].astype("int64")
            # SIMILAR_GAMES 내 중복 제거 (recommended_app_id 기준)
...
# MATRIX 테이블에서 코사인 유사도 데이터 가져오기 (수정됨)
@st.cache_data
    ...
    try:
        ...
        if results:
            ...
            if similar_games:
                ...
                # MATRIX 내 중복 제거 (recommended_app_id 기준, similarity 최대값 유지)
...
if len(selected_tags) < 2:
    ...
    if selected_menu == "홈 대시보드":
        ...
        if title_info:
            ...
            # 추천 게임 목록 (SIMILAR_GAMES 기반 + MATRIX 유사도 추가)
            st.subheader("추천 게임 목록(코사인 유사도 50% 이상은 색으로 표시 됩니다.)")
            ...
            if not similar_games_df.empty:
                ...
                # 병합 후 중복 제거 (recommended_app_id 기준, similarity 최대값 유지)
                ...
                def highlight_similarity(row):
                    if row["similarity"] >= 0.5:
                        return ['background-color: #d4edda'] * len(row)  # 연한 초록색
                    return [''] * len(row)
            ...
            with col9:
                ...
                if positive_reviews:
                    ...
                    if "positive_selection" not in st.session_state:
            ...
            with col10:
                ...
                if negative_reviews:
                    ...
                    if "negative_selection" not in st.session_state:
