...
def color_and_size_by_frequency(word, font_size, position, orientation, random_state=None, frequencies=None, **kwargs):
    ...
    max_count = max(frequencies.values()) if frequencies and frequencies.values() else 1
    ...
    if selected_menu == "홈 대시보드":
        if not filtered_titles:
            ...
            with col5:
                ...
                if tag_counts:
                    font_path = "malgunbd.ttf"  # GitHub 리포지토리 루트에 있는 폰트 파일
                    wordcloud = WordCloud(
                        width=500,
                        height=300,
                    ...
                    with elements("nivo_bar_chart"):
                        with mui.Box(sx={"height": 450}):
                            nivo.Bar(
                                ...
                                theme={
                                    "axis": {
                                        "ticks": {"text": {"fontSize": 14, "fontWeight": "bold"}},
                                        "legend": {"text": {"fontSize": 16, "fontWeight": "bold"}}
                                    },
                                    "labels": {"text": {"fontSize": 14, "fontWeight": "bold"}}
                                },
                                animate=True,
                                motionStiffness=90,
                                motionDamping=15
            ...
            with col10:
                ...
                if not filtered_df.empty:
                    ...
                    wordcloud = WordCloud(
                        ...
                        height=200,
                        background_color="white",
                        font_path="malgunbd.ttf",  # 수정된 부분: malgunbd.ttf 폰트 적용
        ...
        if title_info:
            ...
            with col7:
                ...
                if title_info["positive_keywords"]:
                    positive_tag_counts = {kw: title_info["positive_keywords"].count(kw) for kw in set(title_info["positive_keywords"])}
            ...
            with col8:
                ...
                if title_info["negative_keywords"]:
                    negative_tag_counts = {kw: title_info["negative_keywords"].count(kw) for kw in set(title_info["negative_keywords"])}
                    font_path = "malgunbd.ttf"
