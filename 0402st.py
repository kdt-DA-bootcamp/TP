...
if len(selected_tags) < 2:
    ...
    if selected_menu == "홈 대시보드":
        if not filtered_titles:
            ...
            # 워드 클라우드와 Nivo Bar 차트를 가로로 배치
            col5, col6 = st.columns(2)

            # 동일한 데이터 준비 (선택한 태그 제외한 나머지 태그 빈도)
            tag_counts = {}
            for title in filtered_titles:
                for tag in title["tags"]:
                    if tag not in selected_tags:  # 선택한 태그 제외
            ...
            with col5:
                ...
                if tag_counts:
                    ...
                    def color_and_size_by_frequency(word, font_size, position, orientation, random_state=None, **kwargs):
                        count = tag_counts.get(word, 0)
                    ...
                    def scale_font_size(word, font_size, **kwargs):
                        count = tag_counts.get(word, 0)
                        if count >= max_count * 0.75:
                            return int(font_size * 1.5)
                        return font_size

                    # GitHub에 올려둔 malgunbd.ttf 파일 URL (사용자 본인의 GitHub URL로 변경)
                    FONT_URL = "https://github.com/kdt-DA-bootcamp/TP/blob/main/malgunbd.ttf"

                    # GitHub에서 폰트 다운로드 후 임시 파일로 저장
                    response = requests.get(FONT_URL)
                    if response.status_code == 200:
                        with tempfile.NamedTemporaryFile(delete=False, suffix=".ttf") as temp_font:
                            temp_font.write(response.content)
                            font_path = temp_font.name  # 임시 폰트 파일 경로
                    else:
                        font_path = None  # 폰트 다운로드 실패 시 기본 폰트 사용
                    ...
                    wordcloud = WordCloud(
                        ...
                        font_path=font_path,  # 배포 환경에서도 사용 가능
                        color_func=color_and_size_by_frequency,
                        prefer_horizontal=0.9,
                        scale=2,
                    ).generate_from_frequencies(tag_counts)  # 전체 tag_counts 사용

                    wordcloud = wordcloud.recolor(color_func=color_and_size_by_frequency)
                    for word in wordcloud.words_:
                        wordcloud.words_[word] = scale_font_size(word, wordcloud.words_[word])
            ...
            with col6:
                st.subheader(f"선택한 태그 외 상위 10개 분포 (바 차트)")
                # 상위 10개 태그만 추출
                top_n = 10
                ...
                if chart_data and sum(d["count"] for d in chart_data) > 0:
                    with elements("nivo_bar_chart"):
                        with mui.Box(sx={"height": 450}):
                            nivo.Bar(
                                data=chart_data,
                                keys=["count"],  # 바의 값으로 사용할 키
                                indexBy="tag",   # X축에 사용할 태그 이름
                                margin={"top": 40, "right": 40, "bottom": 100, "left": 60},
                                padding={0.3},   # 바 간격
                                valueScale={"type": "linear"},
                                indexScale={"type": "band", "round": True},
                                colors={"scheme": "category10"},  # 색상 스킴
                                borderColor={"from": "color", "modifiers": [["darker", 1.6]]},
                                axisBottom={
                                    "tickSize": 5,
                                    "tickPadding": 5,
                                    "tickRotation": -45,  # 태그 이름이 길 경우 회전
                                    "legend": "태그",
                                ...
                                theme={
                                    "axis": {
                                        "ticks": {"text": {"fontSize": 14, "fontWeight": "bold"}},  # 축 레이블 굵게
                                        "legend": {"text": {"fontSize": 16, "fontWeight": "bold"}}  # 축 제목 굵게
                                    },
                                    "labels": {"text": {"fontSize": 14, "fontWeight": "bold"}}  # 바 위 값 레이블 굵게
                                },
                                animate=True,
                                motionStiffness=90,
                                motionDamping=15
            ...
            # 나머지 "선택한 장르별 타이틀 리스트" 부분은 그대로 유지
            st.markdown("""
                <style>
                /* st.data_editor의 전체 표 스타일 */
                div[data-testid="stHorizontalBlock"] div[data-testid="stTable"] {
                    width: 100% !important;
                    overflow-x: auto !important;  /* 수평 스크롤 허용 */
                }
                /* 선택, app_id, rating 열 최소화 */
                ...
                div[data-testid="stHorizontalBlock"] div[data-testid="stTable"] td:nth-child(5) {
                    min-width: 800px !important;  /* 태그 열을 더 길게 */
                    overflow-x: scroll !important;  /* 수평 스크롤 강제 적용 */
                    white-space: nowrap !important;  /* 태그가 한 줄로 나열됨 */
                    padding: 0 !important;  /* 불필요한 패딩 제거 */
                }
                /* 태그 열 내부의 태그 리스트 스타일 */
                div[data-testid="stHorizontalBlock"] div[data-testid="stTable"] td:nth-child(5) > div {
                    display: inline-block !important;  /* flex 대신 inline-block으로 단순화 */
                    overflow-x: scroll !important;  /* 태그 리스트 자체에도 스크롤 강제 적용 */
